...
function createClientPluginBody(React) {
  ...
  const MOTION_CSS = [
    ...
    ".dsl-row-act:hover{opacity:1!important;background:rgba(128,128,128,.18)!important}",
    // A small label for icon-only buttons, drawn to the LEFT of the icon so
    // it stays inside the row and is never clipped by the list's scroll box.
    ".dsl-tip{position:relative}",
    ".dsl-tip:hover::after,.dsl-tip:focus-visible::after{content:attr(data-tip);position:absolute;right:calc(100% + 6px);top:50%;transform:translateY(-50%);white-space:nowrap;padding:2px 8px;border-radius:6px;font-size:11px;font-weight:600;line-height:18px;font-family:inherit;background:#2b2b30;color:#f2f2f2;border:1px solid rgba(255,255,255,.14);box-shadow:0 4px 12px rgba(0,0,0,.3);pointer-events:none;z-index:5}",
    "@media (prefers-color-scheme: light){.dsl-tip:hover::after,.dsl-tip:focus-visible::after{background:#fff;color:#111;border-color:rgba(0,0,0,.12);box-shadow:0 4px 12px rgba(0,0,0,.12)}}",
  ...
  /**
   * "Are you sure?" for sending now and cancelling. A dimmed backdrop over the
   * whole window with one small card: what will happen, the message itself,
   * and two buttons — the safe one first. The backdrop or Esc (handled by the
   * component that shows it) dismisses it.
   *
   * @param {object} p { title, note, preview, confirmLabel, keepLabel, danger, onConfirm, onClose, isDark }
   */
  function confirmDialog(p) {
    const stopIt = (e) => { if (e && typeof e.stopPropagation === "function") e.stopPropagation(); };
    const tone = p.danger ? "#ef4444" : "#3b82f6";
    const card = p.isDark ? "#1f1f22" : "#fff";
    const hair = p.isDark ? "rgba(255,255,255,.14)" : "rgba(0,0,0,.10)";
    const btn = (key, label, onClick, primary) => h("button", {
      key, type: "button", autoFocus: !!primary,
      onClick: (e) => { stopIt(e); onClick(); },
      style: {
        cursor: "pointer", borderRadius: 999, padding: "5px 14px", fontSize: 12.5, fontWeight: primary ? 700 : 600,
        fontFamily: "inherit", lineHeight: "18px",
        border: primary ? "none" : "1px solid rgba(128,128,128,.4)",
        background: primary ? tone : "transparent", color: primary ? "#fff" : "inherit",
      },
    }, label);
    return h("div", {
      key: "confirm", "data-plugin": "dsh-schedule-later-confirm", role: "presentation",
      onClick: (e) => { stopIt(e); p.onClose(); },
      style: {
      ...
      role: "alertdialog", "aria-modal": "true", "aria-label": p.title, className: "dsl-pop",
      onClick: stopIt,
      style: {
        width: 380, maxWidth: "100%", boxSizing: "border-box", borderRadius: 14, padding: "16px 16px 14px",
        background: card, color: p.isDark ? "#eee" : "#111", border: "1px solid " + hair,
        boxShadow: "0 16px 40px rgba(0,0,0,.35)", fontSize: 13, lineHeight: "19px",
      },
    }, [
      h("div", { key: "hd", style: { display: "flex", gap: 10, alignItems: "flex-start" } }, [
        h("span", {
          key: "i", "aria-hidden": "true",
          style: {
            flex: "none", width: 28, height: 28, borderRadius: 999, display: "inline-flex", alignItems: "center", justifyContent: "center",
            background: p.iconTone === "day" ? "rgba(251,191,36,.18)" : p.iconTone === "night" ? "rgba(59,130,246,.18)"
              : p.danger ? "rgba(239,68,68,.15)" : "rgba(59,130,246,.15)",
            color: p.iconTone === "day" ? (p.isDark ? "#fcd34d" : "#b45309") : p.iconTone === "night" ? (p.isDark ? "#93c5fd" : "#2563eb") : tone,
            fontWeight: 800, fontSize: 15,
          },
        }, p.icon === "send" ? SendGlyph(14) : "!"),
        h("div", { key: "t", style: { minWidth: 0 } }, [
          h("div", { key: "a", style: { fontWeight: 700, fontSize: 14 } }, p.title),
          p.note ? h("div", { key: "b", style: { marginTop: 3, opacity: .75 } }, p.note) : null,
        ]),
      ]),
      p.preview
        ? h("div", {
            key: "pv", "data-confirm-preview": "",
            style: {
              margin: "12px 0 0", padding: "8px 10px", borderRadius: 8, border: "1px solid " + hair,
              background: p.isDark ? "rgba(255,255,255,.04)" : "rgba(0,0,0,.035)",
              fontFamily: "monospace", fontSize: 12, lineHeight: "17px", whiteSpace: "pre-wrap", wordBreak: "break-word",
              display: "-webkit-box", WebkitLineClamp: 4, WebkitBoxOrient: "vertical", overflow: "hidden",
            },
          }, p.preview)
        : null,
      h("div", { key: "ac", style: { display: "flex", justifyContent: "flex-end", gap: 8, marginTop: 14 } }, [
        btn("keep", p.keepLabel || "Keep it scheduled", p.onClose, false),
        btn("go", p.confirmLabel, () => { p.onClose(); p.onConfirm(); }, true),
      ]),
    ]));
  }

  /** The dialog for one message, sending now or cancelling it. */
  function askFor(kind, t, chatTitle) {
    const when = new Date(t.sendAt);
    const at = clock12(when);
    const time = at.h12 + ":" + at.mm + " " + at.ampm;
    const where = chatTitle ? "“" + chatTitle + "”" : "its chat";
    if (kind === "send") {
      return {
        title: "Send this message now?",
        note: "It goes into " + where + " right away, not at " + time + ", and the model will reply to it.",
        // The button is red like Cancel: once sent it cannot be taken back.
        // The paper plane keeps the send time's colour — amber by day, blue by night.
        preview: t.content, confirmLabel: "Send now", danger: true, icon: "send",
        iconTone: isDaytime(hourOf(when)) ? "day" : "night",
      };
    }
    return {
      title: "Cancel this scheduled message?",
      note: "It will not be sent at " + time + ". This can\u2019t be undone.",
      preview: t.content, confirmLabel: "Cancel message", danger: true,
    };
  }

  /** Paper plane: "Send now". */
  function SendGlyph(size) {
    return h("svg", {
      width: size, height: size, viewBox: "0 0 24 24", fill: "none",
      stroke: "currentColor", strokeWidth: 2, strokeLinecap: "round", strokeLinejoin: "round",
      "aria-hidden": "true", style: { flex: "none", display: "block" },
    }, [
      h("path", { key: "a", d: "M22 2 11 13" }),
      h("path", { key: "b", d: "M22 2 15 22 11 13 2 9 22 2" }),
    ]);
  }

  /** Clock face: "Change time". */
  function ClockGlyph(size) {
    return h("svg", {
      width: size, height: size, viewBox: "0 0 24 24", fill: "none",
      stroke: "currentColor", strokeWidth: 2, strokeLinecap: "round", strokeLinejoin: "round",
      "aria-hidden": "true", style: { flex: "none", display: "block" },
   ...
   */
  function whenPill(sendAt, now, isDark, opts) {
    const withDay = !(opts && opts.day === false);
    ...
    return h("span", {
      ...
      withDay ? h("span", { key: "d" }, shortDay(sendAt, now)) : null,
      h("span", {
        key: "p",
        style: {
  ...
  function ScheduledDock(props) {
    ...
    const mobile = isMobile();
    // The pending "Cancel this message?" for the in-chat list.
    const [ask, setAsk] = React.useState(null);
    React.useEffect(() => {
      if (!ask || typeof document === "undefined" || !document.addEventListener) return undefined;
      const onKey = (e) => { if (e.key === "Escape") { setAsk(null); setTick((n) => n + 1); } };
    ...
    const cancelBtn = (t) => h("button", {
      key: "x", type: "button",
      onClick: () => {
        setAsk({ ...askFor("cancel", t, ""), onConfirm: () => { core.cancelTask(t.id); } });
        setTick((n) => n + 1);
      },
      style: {
    ...
    return h("div", { "data-plugin": "dsh-schedule-later-dock", style: rowStyle }, [
      ...
      visible.length
        ...
        : null,
      ask ? confirmDialog({ ...ask, isDark, onClose: () => { setAsk(null); setTick((n) => n + 1); } }) : null,
    ]);
  }
  ...
  function ScheduledTasksPanel(props) {
    ...
    const mobile = isMobile();
    // dsh passes wide: false when the sidebar is collapsed to its icon rail:
    // show the stopwatch alone then, like Settings does.
    const wide = props.wide !== false;
    ensureStyles();

    // Close like a menu: a press anywhere outside the entry and its panel, or
    // Esc. The panel is position: fixed but still a DOM child of the root, so
    // one contains() covers both. Capture phase, so a handler elsewhere that
    // stops propagation cannot keep it open. A click into an iframe (the graft
    // viz tab, say) never reaches this document; it shows up as the window
    // losing focus to that iframe, so that closes it too.
    const rootRef = React.useRef(null);
    // Row actions: which row is being re-timed, which "Send now" is armed
    // (a second click sends), which chat's "Cancel all" is asking.
    const [edit, setEdit] = React.useState(null); // { id, value, error }
    // The confirmation being asked: { title, note, preview, confirmLabel, danger, onConfirm }.
    const [ask, setAskState] = React.useState(null);
    const askRef = React.useRef(null);
    const setAsk = (v) => { askRef.current = v; setAskState(v); };
    // The message whose details card is open, by id.
    const [detail, setDetailState] = React.useState(null);
    const detailRef = React.useRef(null);
    const setDetail = (id) => { detailRef.current = id; setDetailState(id); };
    const [copied, setCopied] = React.useState(false);

    // dsh's sidebar footer is a flex ROW that does not wrap, and every entry
    // asks for 100% of it, so a second entry is pushed off its right edge.
    // A CSS rule keyed on the footer's exact nesting broke once dsh's markup
    // shifted, so this finds the footer from the entry itself: the first
    // ancestor that actually lays out (display: contents wrappers skipped).
    // If that is a non-wrapping flex row, it is told to wrap. Re-checked on
    // every render, so a footer dsh re-creates is fixed again.
    React.useEffect(() => {
    ...
    React.useEffect(() => {
      ...
      const close = () => { setAsk(null); setDetail(null); setEdit(null); setOpen(false); setTick((n) => n + 1); };
      const onDown = (e) => {
        const root = rootRef.current;
        if (root && typeof root.contains === "function" && !root.contains(e.target)) close();
      };
      const onKey = (e) => {
        if (e.key !== "Escape") return;
        if (askRef.current) { setAsk(null); setTick((n) => n + 1); return; }
        if (detailRef.current) { setDetail(null); setEdit(null); setTick((n) => n + 1); } else close();
      };
      const onBlur = () => {
        setTimeout(() => { if (document.activeElement && document.activeElement.tagName === "IFRAME") close(); }, 0);
      };
      document.addEventListener("pointerdown", onDown, true);
      document.addEventListener("keydown", onKey);
      if (typeof window !== "undefined" && window.addEventListener) window.addEventListener("blur", onBlur);
      return () => {
        document.removeEventListener("pointerdown", onDown, true);
        document.removeEventListener("keydown", onKey);
        if (typeof window !== "undefined" && window.removeEventListener) window.removeEventListener("blur", onBlur);
      };
    }, [open]);
    ...
    // Settings' own row, measured on the live page: a 50px row holding a 42px
    // button that runs 2px past the row on each side, padding 0 10px 0 8px.
    const layerStyle = {
      flex: "0 0 100%", minWidth: 0, alignItems: "center", height: 50,
      margin: 0, display: "flex", position: "relative",
    };
    const entryBtn = {
      width: "calc(100% + 4px)", margin: "0 -2px", minWidth: 0, height: 42, boxSizing: "border-box",
      color: "var(--dsw-alias-label-primary)",
      cursor: "pointer", background: "0 0", border: "none", borderRadius: 12,
      alignItems: "center", gap: 8, padding: "0 10px 0 8px", fontFamily: "inherit",
      fontSize: 14, display: "inline-flex", overflow: "hidden",
    };
    const iconBox = { display: "inline-flex", alignItems: "center", flex: "none" };
    // The collapsed rail's row and button (sized against Settings' on the live page).
    const collapsedLayer = { display: "flex", justifyContent: "center", alignItems: "center", position: "relative", width: "100%", height: 50 };
    const collapsedBtn = {
      position: "relative", width: 36, height: 36, boxSizing: "border-box", padding: 0, border: "none", borderRadius: 10,
      background: "0 0", color: "var(--dsw-alias-label-primary)", cursor: "pointer",
    ...
    const confirmSend = (t) => setAsk({
      ...askFor("send", t, t.sessionExists ? t.sessionTitle : ""),
      onConfirm: () => { if (detailRef.current === t.id) setDetail(null); core.rescheduleTask(t.id, "now").catch(() => {}); },
    });
    const confirmCancel = (t) => setAsk({
      ...askFor("cancel", t, t.sessionExists ? t.sessionTitle : ""),
      onConfirm: () => { if (detailRef.current === t.id) setDetail(null); core.cancelTask(t.id).catch(() => {}); },
    });
    const cancelBtn = (t) => h("button", {
      key: "x", type: "button",
      onClick: (e) => {
    ...
    const smallBtn = (key, label, onClick, extra) => h("button", {
      key, type: "button", onClick: (e) => { stop(e); onClick(); setTick((n) => n + 1); },
      style: {
        cursor: "pointer", border: "1px solid rgba(128,128,128,.4)", borderRadius: 999, padding: "0 8px",
        fontSize: 11, lineHeight: "18px", background: "transparent", color: "inherit", flexShrink: 0, whiteSpace: "nowrap",
        ...(mobile ? { minHeight: TOUCH_MIN, boxSizing: "border-box" } : {}),
        ...(extra || {}),
      },
    }, label);
    const iconBtn = (key, title, glyph, onClick) => h("button", {
      key, type: "button", "aria-label": title, "data-tip": title, className: "dsl-row-act dsl-tip",
      onClick: (e) => { stop(e); onClick(); setTick((n) => n + 1); },
      style: {
        cursor: "pointer", border: "none", borderRadius: 6, width: 22, height: 20, padding: 0,
        display: "inline-flex", alignItems: "center", justifyContent: "center",
        background: "transparent", color: "inherit", opacity: .75, flexShrink: 0,
        ...(mobile ? { minHeight: TOUCH_MIN, minWidth: TOUCH_MIN } : {}),
      },
    }, glyph);

    /** Pending rows by chat, chats in order of their soonest message. */
    function groupByChat(rows) {
      const groups = [];
      const byKey = new Map();
      for (const t of rows) {
        const key = t.sessionExists ? String(t.conversationId) : "gone:" + String(t.conversationId || "");
        let g = byKey.get(key);
        if (!g) {
          g = { key, title: t.sessionTitle, exists: !!t.sessionExists, tasks: [] };
          byKey.set(key, g);
          groups.push(g);
        }
        g.tasks.push(t);
    ...
    /** "Cancel all" for one chat, through the confirmation dialog. */
    function cancelAllCtl(g) {
      return smallBtn("ca", "Cancel all", () => setAsk({
        title: "Cancel all " + g.tasks.length + " scheduled messages?",
        note: "None of them will be sent" + (g.exists ? " to \u201c" + g.title + "\u201d" : "") + ". This can\u2019t be undone.",
        preview: g.tasks.map((t) => "\u2022 " + String(t.content).split(/\r?\n/, 1)[0]).join("\n"),
        confirmLabel: "Cancel " + g.tasks.length + " messages", keepLabel: "Keep them", danger: true,
        onConfirm: () => {
          if (g.tasks.some((t) => t.id === detailRef.current)) setDetail(null);
          for (const t of g.tasks) core.cancelTask(t.id).catch(() => {});
        },
      }), { marginLeft: "auto" });
    }

    /** A chat's rows, with a day heading wherever the day changes (Today needs none). */
    function groupRows(g) {
      const nowMs = Date.now();
      const out = [];
      let prev = null;
      for (const t of g.tasks) {
        const day = shortDay(t.sendAt, nowMs);
        if (day !== prev && (prev !== null || day !== "Today")) {
          out.push(h("div", {
            key: "d:" + t.id, "data-day": day,
            style: { fontSize: 10.5, fontWeight: 700, letterSpacing: ".4px", textTransform: "uppercase", opacity: .55, margin: "6px 2px 3px" },
          }, day));
        }
        prev = day;
        out.push(taskRow(t, nowMs));
      }
      return out;
    }

    const saveEdit = (t) => {
      const at = edit ? new Date(edit.value).getTime() : NaN;
      if (!Number.isFinite(at) || at <= Date.now()) {
        setEdit({ ...edit, error: "Pick a time in the future." });
        return;
      }
      setEdit(null);
      core.rescheduleTask(t.id, at).catch(() => {});
    };

    function editRow(t) {
      return h("div", { key: "e", onClick: stop, style: { display: "flex", alignItems: "center", gap: 6, flexWrap: "wrap" } }, [
        h("input", {
          key: "i", type: "datetime-local", value: edit.value, "aria-label": "New send time",
          min: toLocalInput(Date.now() + 60_000),
          onChange: (e) => { setEdit({ ...edit, id: t.id, value: e.target.value, error: null }); setTick((n) => n + 1); },
          onKeyDown: (e) => {
            if (e.key === "Enter") saveEdit(t);
            if (e.key === "Escape") { stop(e); setEdit(null); }
            setTick((n) => n + 1);
          },
          style: {
            font: "inherit", fontSize: 12, padding: "2px 6px", borderRadius: 6, border: "1px solid rgba(128,128,128,.4)",
            background: "transparent", color: "inherit", colorScheme: isDark ? "dark" : "light", minWidth: 0,
          },
        }),
        smallBtn("sv", "Save", () => saveEdit(t), { border: "none", background: "#3b82f6", color: "#fff", fontWeight: 700 }),
        smallBtn("bk", "Back", () => setEdit(null)),
        edit.error ? h("span", { key: "er", style: { color: "#dc2626", width: "100%" } }, edit.error) : null,
      ]);
    }

    function taskRow(t, nowMs) {
      const sky = skyRow(t.sendAt, nowMs);
      const editing = !!(edit && edit.id === t.id && edit.where !== "detail");
      return h("div", {
        key: t.id, "data-task": t.id, className: sky.className,
        onClick: () => { if (!editing) { setDetail(detail === t.id ? null : t.id); setTick((n) => n + 1); } },
        "aria-expanded": detail === t.id,
        style: {
          display: "flex", flexDirection: "column", gap: 3, padding: "6px 8px", marginBottom: 4,
          borderRadius: 8, ...sky.style,
          ...(detail === t.id ? { boxShadow: "0 0 0 2px #3b82f6" } : {}),
          cursor: "pointer",
          opacity: t.sessionExists ? 1 : 0.5,
          maxWidth: "100%", boxSizing: "border-box",
        },
      }, [
        // The whole message, clamped to two lines; hover shows all of it.
        h("div", {
          key: "c", "data-content": "",
          style: {
            whiteSpace: "pre-wrap", fontFamily: "monospace", wordBreak: "break-word",
            display: "-webkit-box", WebkitLineClamp: 2, WebkitBoxOrient: "vertical", overflow: "hidden",
          },
        }, t.content || "(empty)"),
        editing
          ? editRow(t)
          : h("div", { key: "m", style: { display: "flex", alignItems: "center", gap: 6, minWidth: 0 } }, [
              whenPill(t.sendAt, nowMs, isDark, { day: false }),
              h("span", { key: "cd", style: { color: "#3b82f6", fontWeight: 600, display: "inline-flex", alignItems: "center", gap: 4, minWidth: 0, overflow: "hidden", whiteSpace: "nowrap" } },
                [HourglassIcon(11), h("span", { key: "t" }, sendsIn(t.sendAt, nowMs))]),
              h("span", { key: "sp", style: { marginLeft: "auto", display: "inline-flex", alignItems: "center", gap: 2, flex: "none" } }, [
                iconBtn("sn", "Send now", SendGlyph(13), () => { setEdit(null); confirmSend(t); }),
                iconBtn("ed", "Change time", ClockGlyph(13), () => setEdit({ id: t.id, value: toLocalInput(t.sendAt), error: null, where: "row" })),
                cancelBtn(t),
              ]),
            ]),
      ]);
    ...
    /** "Saturday 27 September 2026, 2:35 AM" in the browser's locale. */
    const longWhen = (ms) => new Date(ms).toLocaleString(undefined, {
    ...
    function detailCard() {
      const t = annotated.find((x) => x.id === detail);
      if (!t) return null;
      const nowMs = Date.now();
      const byAssistant = t.meta && t.meta.scheduledBy === "assistant";
      const editingHere = !!(edit && edit.id === t.id && edit.where === "detail");
      const narrow = mobile || (typeof window !== "undefined" && window.innerWidth < 760);
      const surface = isDark ? "#1c1c1e" : "#fff";
      const hair = isDark ? "rgba(255,255,255,.14)" : "rgba(0,0,0,.10)";
      const label = (key, text) => h("div", { key, style: { fontSize: 10.5, fontWeight: 700, letterSpacing: ".4px", textTransform: "uppercase", opacity: .55, margin: "10px 0 4px" } }, text);
      const copy = () => {
        try {
          const done = () => { setCopied(true); setTick((n) => n + 1); setTimeout(() => { setCopied(false); setTick((n) => n + 1); }, 1500); };
          if (typeof navigator !== "undefined" && navigator.clipboard && navigator.clipboard.writeText) navigator.clipboard.writeText(t.content).then(done, () => {});
        } catch { /* no clipboard: nothing to do */ }
      };
      return h("div", {
        key: "detail", "data-plugin": "dsh-schedule-later-detail", role: "dialog", "aria-label": "Scheduled message details",
        className: "dsl-pop",
        onClick: stop,
        style: {
          // Beside the list on a wide window; over it on a narrow one.
          position: "fixed", bottom: 56, zIndex: 51,
          left: narrow ? (mobile ? 8 : (wide ? 12 : 64)) : (wide ? 12 : 64) + 340 + 8,
          width: mobile ? "calc(100vw - 16px)" : 380, maxWidth: "calc(100vw - 24px)", maxHeight: "70vh",
          display: "flex", flexDirection: "column", boxSizing: "border-box",
          borderRadius: 12, border: "1px solid " + hair, background: surface, color: isDark ? "#eee" : "#111",
          boxShadow: "0 12px 32px rgba(0,0,0,.28)", fontSize: 12, overflow: "hidden",
        },
      }, [
        // the sky of the send time, as a header band
        h("div", { key: "hd", className: skyRow(t.sendAt, nowMs).className, style: { ...skyRow(t.sendAt, nowMs).style, borderRadius: 0, border: "none", padding: "10px 12px", display: "flex", alignItems: "center", gap: 8 } }, [
          whenPill(t.sendAt, nowMs, isDark),
          h("span", { key: "cd", style: { color: "#3b82f6", fontWeight: 600, display: "inline-flex", alignItems: "center", gap: 4 } }, [HourglassIcon(11), h("span", { key: "t" }, sendsIn(t.sendAt, nowMs))]),
          h("button", {
            key: "x", type: "button", "aria-label": "Close details", onClick: (e) => { stop(e); setDetail(null); setEdit(null); setTick((n) => n + 1); },
            style: { cursor: "pointer", marginLeft: "auto", border: "none", background: "transparent", color: "inherit", fontSize: 14, lineHeight: 1 },
          }, "✕"),
        ]),
        h("div", { key: "body", className: "dsl-dock-list", style: { padding: "2px 12px 10px", overflowY: "auto", minHeight: 0 } }, [
          h("div", { key: "when", style: { marginTop: 8, fontWeight: 600 } }, longWhen(t.sendAt)),
          h("div", { key: "who", style: { marginTop: 4, display: "flex", alignItems: "center", gap: 6, flexWrap: "wrap", opacity: .8 } }, [
            h("span", {
              key: "b", style: {
                padding: "0 7px", borderRadius: 999, fontSize: 11, fontWeight: 700, lineHeight: "18px",
                background: byAssistant ? "rgba(168,85,247,.18)" : "rgba(59,130,246,.16)", color: byAssistant ? "#c084fc" : "#60a5fa",
              },
            }, byAssistant ? "Scheduled by the assistant" : "Scheduled by you"),
            t.createdAt ? h("span", { key: "c", title: formatLocalTime(t.createdAt) }, ago(t.createdAt)) : null,
          ]),

          label("lc", "Chat"),
          h("div", { key: "chat", style: { display: "flex", alignItems: "center", gap: 8, minWidth: 0 } }, [
            h("span", { key: "t", style: { fontWeight: 600, minWidth: 0, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap", opacity: t.sessionExists ? 1 : .6 } },
              t.sessionExists ? t.sessionTitle : "Chat no longer exists"),
            t.sessionExists ? smallBtn("open", "Open chat", () => jump(t), { marginLeft: "auto" }) : null,
          ]),

          h("div", { key: "lm", style: { display: "flex", alignItems: "center", margin: "10px 0 4px" } }, [
            h("span", { key: "l", style: { fontSize: 10.5, fontWeight: 700, letterSpacing: ".4px", textTransform: "uppercase", opacity: .55 } }, "Message"),
            h("span", { key: "n", style: { opacity: .45, marginLeft: 6 } }, t.content.length.toLocaleString() + " chars"),
            smallBtn("cp", copied ? "Copied" : "Copy", copy, { marginLeft: "auto" }),
          ]),
          h("pre", {
            key: "msg", "data-detail-content": "", className: "dsl-dock-list",
            style: {
              margin: 0, padding: "8px 10px", borderRadius: 8, maxHeight: "32vh", overflow: "auto",
              background: isDark ? "rgba(255,255,255,.04)" : "rgba(0,0,0,.035)", border: "1px solid " + hair,
              whiteSpace: "pre-wrap", wordBreak: "break-word", fontFamily: "monospace", fontSize: 12, lineHeight: "18px",
            },
          }, t.content),
          byAssistant
            ? h("div", { key: "mk", style: { marginTop: 6, opacity: .6 } }, "It arrives starting with [Scheduled by the assistant], so it is never mistaken for you typing.")
            : null,
          t.attempts > 0 || t.lastError
            ? h("div", { key: "err", style: { marginTop: 8, padding: "6px 8px", borderRadius: 8, background: "rgba(245,158,11,.12)", color: isDark ? "#fcd34d" : "#b45309", wordBreak: "break-word" } },
                "Delivery tried " + (t.attempts || 0) + " time" + (t.attempts === 1 ? "" : "s") + (t.lastError ? " — last error: " + t.lastError : "") + ". It will try again.")
            : null,
        ]),

        // actions
        h("div", { key: "ft", style: { borderTop: "1px solid " + hair, padding: "8px 12px", display: "flex", alignItems: "center", gap: 6, flexWrap: "wrap" } },
          editingHere
            ? [editRow(t)]
            : [
                smallBtn("sn", "Send now", () => confirmSend(t)),
                smallBtn("ed", "Change time", () => setEdit({ id: t.id, value: toLocalInput(t.sendAt), error: null, where: "detail" })),
                smallBtn("cx", "Cancel message", () => confirmCancel(t),
                  { marginLeft: "auto", color: "#ef4444", borderColor: "rgba(239,68,68,.45)" }),
              ]),
      ]);
    }
    ...
    const footer = open
      ? h("div", {
          ...
          style: {
            // Beside the collapsed rail rather than over it.
            position: "fixed", bottom: 56, left: wide ? 12 : 64, zIndex: 50,
          ...
          !all.length && !err
            ...
            : null,
          groupByChat(annotated).map((g) => h("div", { key: "g:" + g.key, "data-group": g.key, style: { marginBottom: 6 } }, [
            h("div", { key: "hd", style: { display: "flex", alignItems: "center", gap: 6, padding: "4px 2px", minWidth: 0 } }, [
              h("button", {
                key: "t", type: "button",
                onClick: () => { if (g.exists) jump(g.tasks[0]); },
                title: g.exists ? "Open this chat" : "Chat no longer exists — its messages can only be cancelled",
                style: {
                  cursor: g.exists ? "pointer" : "default", border: "none", background: "transparent", color: "inherit",
                  padding: 0, font: "inherit", fontWeight: 700, textAlign: "left", opacity: g.exists ? 1 : .6,
                  minWidth: 0, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap",
                },
              }, g.exists ? g.title : "Chat no longer exists"),
              h("span", { key: "n", style: { opacity: .5, flex: "none" } }, String(g.tasks.length)),
              g.tasks.length > 1 ? cancelAllCtl(g) : null,
            ]),
            ...groupRows(g),
          ])),
          overflow > 0
    ...
    const toggle = () => { setOpen(!open); setTick((n) => n + 1); };
    const hoverCss = h("style", { key: "css" }, '.ssb_badge:hover{background:var(--dsw-alias-interactive-bg-hover-solid)}');

    if (!wide) {
      // Collapsed rail: the stopwatch alone, the count as a small corner badge.
      return h("div", { ref: rootRef, "data-plugin": "dsh-schedule-later-sidebar", style: collapsedLayer }, [
        hoverCss,
        h("button", {
          key: "btn", type: "button", className: "ssb_badge", onClick: toggle,
          title: all.length ? `Scheduled messages (${all.length})` : "Scheduled messages",
          "aria-label": "Scheduled messages", style: collapsedBtn,
        }, [
          h("span", { key: "i", style: iconBox }, StopwatchIcon(16, all.length ? "slow" : "")),
          all.length
            ? h("span", {
                key: "b", "data-badge": all.length,
                style: {
                  position: "absolute", top: 1, right: 1, minWidth: 14, height: 14, padding: "0 3px", boxSizing: "border-box",
                  borderRadius: 999, background: "#3b82f6", color: "#fff", fontSize: 9, fontWeight: 700, lineHeight: "14px", textAlign: "center",
                },
              }, all.length > 99 ? "99+" : String(all.length))
            : null,
        ]),
        footer,
        open && detail ? detailCard() : null,
        open && ask ? confirmDialog({ ...ask, isDark, onClose: () => { setAsk(null); setTick((n) => n + 1); } }) : null,
      ]);
    }

    return h("div", { ref: rootRef, "data-plugin": "dsh-schedule-later-sidebar", style: layerStyle }, [
      hoverCss,
      h("button", {
        key: "btn", type: "button",
        className: "ssb_badge",
  ...
  return function buildPlugin({ stateRoutePath, fetchImpl }) {
    ...
    // PATCH moves a pending task; when === "now" sends it at once.
    const doReschedule = async (id, when) => {
      const res = await doFetch(schedulePath + "?id=" + encodeURIComponent(id), {
        method: "PATCH", headers: { "content-type": "application/json", accept: "application/json" },
        body: JSON.stringify(when === "now" ? { now: true } : { sendAt: when }),
      });
      return res.ok;
    };
    ...
    panelCore.onChanged = () => { void core.refresh(); };
      ...
      rescheduleSchedule: doReschedule,
    });
